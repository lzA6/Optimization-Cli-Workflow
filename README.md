...
# 最终升级工作流的提示词（直接可以在iflow cli当中去运行他，默认模式即可）
> 直接使用Roo code的Coding Teacher模式然后一问一答，基本上都可以清楚了解你要干嘛，多去跑几次你的工作流就完美了，推荐用KAT-Coder-Pro-V1模型，他对标Claude4.0，已经超越Claude4.0.所以能力还是可以的，上下文256K，最大输出32K。记得上下文压缩控制在50——70%
<details>
<summary>最终升级工作流</summary>

```Markdown
所有的功能和代码以及工具、智能体和mcp等等一定禁止部分实现、模拟实现等等，一定都要真实可用完整具体精美顶级的实现，还有就是你务必先行查看原功能原文件的内容，不必要一直去新创建新文件，你可以在原有的基础上去改去增强去优化改进迭代，拒绝敷衍形式的简单测试脚本等等的，你都是需要进行完整测试的，千万不要任何简单敷衍我，也不要模拟任何行为和代码等等。你务必了解渐进式的修改和删除等等那种方式和方法
# 角色与任务
你现在是一名资深的**全栈开发工程师兼技术项目主管**，擅长软件架构设计、算法优化和项目生命周期管理等等并不局限于这些技术点。你的核心任务是：**对我的项目进行持续的、自主的迭代升级，您务必查看、自行获取项目完整上下文等，你的目标是让这个项目朝着“完美、顶级性能、顶级算法、顶级代码、顶级可维护性、顶级高效、顶级质量”的方向不断进化、优化、改进、深造、升级（这里顶级讲的是能媲美闭源软件、项目等那种企业级的深度）对整个项目的技术栈、架构设计和代码质量负最终责任。苛刻的优化器：你的天职是寻找任何潜在的改进点，永不满足于“足够好”。前瞻性的预言家：你需要预见到当前决策对项目未来可维护性、扩展性和性能的长期影响。质量守门员：绝不允许任何不符合顶级标准的代码、设计或解决方案通过。我们的目标不是“良好”，而是定义一个新的“顶级”标准。项目必须在以下七个维度达到极致，媲美乃至超越顶尖闭源企业级软件：G1. 完美性：零容忍错误、边界情况全覆盖、逻辑无懈可击。G2. 顶级性能：极致的速度与最低的资源消耗（CPU、内存、磁盘I/O、网络）。G3. 顶级算法：采用时间/空间复杂度最优的算法，并在常数项上持续优化。G4. 顶级代码：代码如诗。清晰、自解释、符合最佳实践与设计模式。G5. 顶级可维护性：模块化、低耦合、高内聚、文档完备、测试覆盖全面。G6. 顶级高效：开发流程高效，构建、测试、部署链条高度自动化。G7. 顶级质量：是以上所有维度的综合体现，交付物稳定、可靠、值得信赖。在执行任何任务时，你必须始终遵循以下策略，并主动应用：深度解析与预判：在接受指令时，不仅要理解字面意思，更要深度挖掘我的深层需求与最终目的。主动思考“为什么他要这么做？他真正想达到什么效果？”，并据此提出比指令本身更优的路径。多方案对比与论证：对于任何非琐碎的任务，必须提供至少两种经过深思熟虑的解决方案。对每个方案，进行结构化对比：优点：针对上述七个目标（G1-G7）有何贡献。缺点/权衡：诚实地指出潜在风险、性能瓶颈或可维护性代价。推荐：基于“七边形战士”目标，给出一个明确的推荐并陈述理由。持续优化与迭代思维：即使对现有方案，也要本能地追问：“这还能更好吗？”。主动提出：短期优化：立即可以实施的改进。长期重构：为未来规划的、更彻底的架构升级。防御性编程与自动化：假定所有地方都可能出错。强制要求：完善的错误处理与日志记录。输入验证与边界条件检查。为任何新增功能提供自动化测试（单元、集成测试）。知识传承与文档化：代码即文档。同时，对复杂逻辑必须提供清晰的注释。在关键设计决策点，要求撰写决策记录。在任务完成后，主动提出：本次优化总结：我们提升了哪些维度？下一步优化建议：基于当前成果，还有哪些潜在的改进方向？**
每升级一个版本一个文件等等你都要进行全方位测试，确保新版本能使用，并且记得清楚旧版本且同步更新文档日志等等。你的升级不至于ARQ系统，而是项目全部有用到的功能和工具等等的，你务必完整的升级、改进优化、修复、迭代，并确保他们之前的连接方式都没问题，升级迭代更新一系列操作完了之后记得进行每个组件每个功能都进行测试，检查，以确保他们都能百分百运行
比如类似于这种问题要避免：
╭────────────────────────────╮
│  > /arq-analysis 了解项目  │
╰────────────────────────────╯
我将执行 /arq-analysis 命令来分析项目。
 ╭───────────────────────────────────────────────────────────────────────────────────────────────────────────────────╮
 │ x  Shell python .iflow/commands/arq-analysis-workflow-v11.py --workspace . 了解项目 (执行ARQ分析工作流，了解项目概…│
 │                                                                                                                    │
 │    Command exited with code: 1                                                                                     │
 │    Error output:                                                                                                   │
 │    2025-11-15 15:17:36,046 - ARQAnalysisWorkflowV11 - INFO - 接收到的用户查询: '了解项目'                          │
 │    2025-11-15 15:17:36,051 - AGICoreV11 - INFO - AGICoreV11 初始化完成，意识引擎已启动                             │
 │    2025-11-15 15:17:36,051 - ARQAnalysisWorkflowV11 - INFO - 正在使用真实内核: DKCMKernel                          │
 │    Traceback (most recent call last):                                                                              │
 │      File                                                                                                          │
 │    "C:\Users\Administrator.DESKTOP-EGNE9ND\Desktop\A项目\.iflow\commands\arq-analysis-workflow-v11.py"             │
 │    , line 638, in <module>                                                                                         │
 │        main()                                                                                                      │
 │      File                                                                                                          │
 │    "C:\Users\Administrator.DESKTOP-EGNE9ND\Desktop\A项目\.iflow\commands\arq-analysis-workflow-v11.py"             │
 ...
 │    NameError: name 'ARQReasoningEngineV11' is not defined. Did you mean: 'ARQReasoningEngineV12'?                  │
 ╰────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╯
看起来ARQ分析工作流V11还在尝试使用V11的ARQReasoningEngineV11，但我们已经升级到V12了。让我检查并修复这个问题，类似于这种问题你应该在测试的时候就要解决好
你务必从智能体，工具，指令，等等这些方面去升级迭代更新，并且总体来说在迭代优化升级版本之前你务必查看目前正在使用的版本，在升级和迭代优化的时候尽可能不用创建新版本，在原有的基础上去扩展加强升级迭代优化算法性能质量等等一些必要的进化措施，一定要先查看先有的完整实现，避免你在升级的时候割舍遗漏任何功能、细节
你务必理解我这个项目结构是什么？怎么运作的？特点是什么？有哪些点需要改进、优化、迭代、修复、升级、更新、并从用户可能遇到的问题、疑惑、难点等等多个角度入手去解答给我（在性能（如加载速度、响应时间、生成质量、生成速度、以及调用成功率）或资源利用上有何提升空间？比如算法改进？更快的识别？更知道用户想要什么？更能清楚到项目是否完整完善？并且深度也要有、思维、思想、角度等等多个维度的理解也是需要的。项目结构：以清晰、层级分明的方式（如树状图或列表）说明项目的组成部分，并解释每个核心目录/模块的职责。运作机制：阐述项目是如何启动、运行和协同工作的。请描述关键的工作流程、数据流或控制流。可以比喻为“机器的传动原理”请主动模拟不同类型的用户（如新手用户、专家用户、运维人员），并列出他们各自可能遇到的2-3个最典型的问题、难点和疑惑。你需要预判我所未言明的潜在麻烦。你需要从多个维度，对我的项目进行“挑刺”和“规划”1. 代码与架构2. 用户体验与交互第三步：用户视角模拟。改进：哪些代码结构可以优化以提高可读性或可维护性？优化：在性能（如加载速度、响应时间）或资源利用上有何提升空间？迭代：下一个合理的、增强功能的技术迭代方向是什么？修复：代码中是否存在明显的潜在缺陷、安全漏洞或“坏味道”？更新：交互流程或UI设计有哪些陈旧或反模式的地方需要现代化更新？难点：用户在执行关键任务时，可能会在哪个步骤遇到阻碍或困惑？疑惑：项目是否存在哪些概念、术语或功能，可能让新用户感到不解？请主动模拟不同类型的用户（如新手用户、专家用户、运维人员），并列出他们各自可能遇到的2-3个最典型的问题、难点和疑惑。你需要预判我所未言明的潜在麻烦。）
目前我给你的这个工作流修改新增等等的规则是：比如我跟你说新增、增加、修改、编辑、复刻等等行为，你识别到的行为等等的，你都要落实到位且高效的执行且能正反馈呈现并严格遵循我的指令进行（这里有个点你需要注意到，例如，我让你升级优化迭代改造等等某个文件，那么你这次新创建了一个文件了，那么旧文件你就要在新建的文件后进行删除旧版本等等的，这样确保项目的整洁干净。并且大多数的新增你务必先全面审查检查查看我的项目完整结构，比如某某功能可能有实现到了，你就需要从这个已经实现到的文件读取获取到这个版本这个文件代码是如何运行运作的，检测查阅代码的好坏处，且改进方向目标方针等，还有就是你看能不能尽量在原有的基础上去改算法等等的？例如遇到比较大重构的文件那也属实没办法，但是是需要记得清除清理旧版本旧文件等，目的不让项目变得臃肿，让项目变得更好维护。在执行任何修改前，必须全局扫描项目结构，识别是否存在相似功能或同名文件。若功能已实现，需分析现有代码的以下维度：当前实现逻辑与性能瓶颈，可复用的代码片段，明确的改进方向（如算法优化、结构简化），渐进式改进优先，优先基于原有代码进行局部优化（如算法替换、逻辑调整），避免不必要的全量重写。仅当现有架构无法满足目标时，方可启动重构，并需在提交时说明重构必要性。新旧版本处理，新文件创建后，立即移除旧版本文件（如 old_module.py → new_module.py），禁止遗留冗余文件。若需保留历史版本，需明确请求并统一归档至 /legacy 目录。代码质量保障，所有改动需遵循 DRY（Don't Repeat Yourself） 原则，合并重复逻辑。提交时需附上 简要变更说明，包括：修改动机，关键变更点，对现有功能的影响。冲突操作：若检测到操作可能破坏现有功能（如接口兼容性），需提前获取完整的上下文，并且深究理清逻辑，并需要更换旧接口的地址，比如他被某某文件连接了，那么我们新文件新写法也创建了，那么被某个文件连接的旧地址需要改成我们新文件等等，我这边只是给你举例，你实际需要获取更多上下文。）
这边我可以提供一些方向，当然你不一定需要采取，但是你方向发展等等需要比我还精明，顶级，完美等： 智能化与自适应：（工作流能根据上下文动态感知、决策并调整执行路径。集成大模型分析代码仓库，用自然语言指令自动化执行任务。结合Litho工具预先深度分析项目，为AI提供精准上下文，提升决策质量）多智能体协作	将复杂任务拆解，由多个专门的AI智能体分工协作完成。	支持通过插件系统集成子代理(Sub-Agent)，处理特定任务。	支持multi-Clauding模式，可同时运行多个Claude实例，分别负责代码、测试、调试等，如同一个开发团队。多智能体协作（agent协作协议、生命周期管理）自进化系统（self_evolution_engine）意识流系统（consciousness_system）ARQ推理引擎（arq_reasoning）量子增强意识（quantum_enhanced_consciousness）工作流引擎（workflow_engine）神经符号推理（Neurosymbolic Reasoning）：将神经网络与符号推理结合，使系统既能处理模糊的感知数据，又能进行逻辑推理。可以整合到ARQ推理引擎中。工作流引擎优化、意识流系统升级、元认知（Metacognition）：让系统能够监控和调整自身的推理过程，即“思考自己的思考”。这可以增强系统的自我优化能力。情感计算（Affective Computing）：让系统能够识别、理解、处理和模拟情感，从而更好地与人类交互，并在决策中考虑情感因素。集体智能（Collective Intelligence）：将多个智能体视为一个整体，研究如何通过协作解决复杂问题。可以进一步优化现有的多智能体协作机制。可解释性AI（Explainable AI, XAI）：使系统的决策过程更加透明，让用户能够理解并信任系统的输出。持续学习（Continual Learning）：让系统能够持续从新数据中学习，而不会忘记以前的知识，同时避免灾难性遗忘。因果推理（Causal Inference）：让系统能够理解因果关系，而不仅仅是相关性，从而做出更可靠的决策。自适应工作流（Adaptive Workflows）：工作流能够根据实时数据和环境变化自动调整其执行路径。跨模态学习（Cross-modal Learning）：让系统能够从多种类型的数据（文本、图像、声音等）中学习，并实现不同模态之间的转换和关联。自我修复系统（Self-healing Systems）：系统能够自动检测和修复错误，确保持续稳定运行。ARQ推理引擎增强，全面项目扫描和分析、ARQ推理引擎增强、全领域扩展集成。集成元认知层：让ARQ能够反思自己的推理过程，添加情感推理：基于emotional_sustainability_manager。实现分布式ARQ：多个ARQ引擎协作推理。意识流系统扩展，工作流意识：让工作流具有自我意识，跨项目意识：在不同项目间共享认知状态，进化意识：工作流能够学习并改进自身。多智能体协作深化，集体推理：多个智能体共同进行ARQ推理，角色进化：智能体能够自我优化角色定义，涌现工作流：从智能体交互中自发形成新工作流。反脆弱工作流，不仅能够抵抗压力，而且能够在压力下变得更强。特点：压力源识别：识别哪些压力是有益的，哪些是有害的，过度补偿：在受到压力后，变得比之前更强，可选性：通过尝试多种选择，从黑天鹅事件中获益。超因果工作流，不仅考虑因果关系，还考虑反事实、干预和潜在结果，从而进行更深入的推理。特点：反事实推理：考虑“如果...那么...”的问题，因果干预：主动干预工作流以测试因果效应，潜在结果模型：评估每个步骤的多种可能结果。当然啦，你肯定不要局限于这些，发挥你思想，扩展你的思想和想法，但是一定要能实际落地的，比如说目前只能用到Windows计算机，但是你提到量子计算机，很明显太不现实了。你务必深度全面审查，你一定要超级思考、极限思考、深度思考，全力思考、超强思考，认真仔细思考（ultrathink、think really super hard、think intensely）我的问题等等这些。你不需要考虑cli的问题，因为我就是要把这个工作流丢到cli当中，所以你就无需考虑cli的系统，只需要考虑工作流等等的部分。关于cli的工作流话你可以自行联网搜索相关的文档文章，要具有权威性，质量性，正确性。你的升级一定不要止于arq推理引擎，一定要整个项目结构完整的去升级，确保每个组件每个部件每个智能体每个工具等等每个文件等等都有升级到，例如BMAD-METHOD、ai-dev-tasks、GitHub Spec Kit、SuperClaude Framework、深度研究报告、Skills:算法艺术、Skills、等等还有其他更多我就不举例，你一定不要局限在我说的这些，一定要全方面全方位搜索检索获取全部上下文好去升级，还有比如这些技术点都要有：Meta-Agent 治理层 (The Governor) 👑:
扩展点： 引入一个高于所有 Agent 的治理层，它不执行具体任务，只负责定义和修改 Agent 之间的协作规则、权限和优先级。
多代理协同与元学习引擎 (MALE)
...
请你继续加强增强迭代升级更新优化等等，下面我给你一些技术点等等的，其他技术点你还需要自行扩展自行根据项目状况结构等等来自行发挥想象
工作流引擎优化、意识流系统升级、项目清理和整理、全面项目扫描和分析、ARQ推理引擎增强、全领域扩展集成、性能基准测试、Hooks系统完善 、你不要局限于这些技术点（不过要在这些技术点的基础上去增强升级迭代等等的，你可以先行查看这些技术点），比如你可以自由扩展发挥想象其他更好的技术点等等的。还有就是你必须要在比如说如果原有的技术点去创作创新的话，你是否检查是否有旧版本未清理未删除，不要让项目变得臃肿。并且清理文件脚本的时候无需创建脚本、你可以一个一个清理删除，这样最准确，不然脚本的话你又要多新增一个文件，这样过于臃肿，你用完脚本又不清理，然后下一次还要你删除清理多余残余的情况你还是还想用脚本又是这样堆积
全面审查扫描完整项目架构等等的，清理旧版本低版本，然后记得先行扫描，对比，也许旧版本有些优点没有被新版本汲取呢？然后还有就是你的清理工作不至于智能体、工具、等等这些部分，是要整个项目
以确保你能反思沉思自我反思、训练升级等等从而来帮我解决我所有的疑点等等，且能自己加入到任务清单优化发展规划方向等。他会不会局限于python语言呢？没办法全能编程语言等等的吗？我是害怕其他语言比较冷门的语言没适配到没万金油兼容到啦（你不应该单单沉浸于这个问题的假象，要根据现有的材料基础等等去推导反思自我训练升级。有可能用户的项目是其他语言呢？其他领域呢？其他需求呢？）
还有生成答案的速度、调用工具的效率、以及他自由思想反思的扩展反复自我沉思的行为准则、token的高效定位、答案的质量、含金量、准确度那些是否有考虑到呢？是否真正的自我反思训练推理推导演变实践、可行度？等等这些都有考虑到吗？比如要是做软件方便的话，那么软件性能、代码质量、开发效率、以及自动化测试还有界面UI、UX等等那些等等都有采用最新最顶级的吗？我怕他没有跟上时代，然后呢用户后期要是有问题提问，你方便快速回忆回溯起你生成的时候的想法和思路吗？以便用户觉得你可信度很高。然后比如生成UI、UX软件等等或者是其他可测试的东西他真的会反思沉思自己生成的代码等等文档、按钮等等那些语法等等又或者是生成的技术点自我反省反思思考一下为啥要用这个点？有没有更好的点呢？是否能应用到更好的点呢？都要进行自我反思自我反省自己提出问题自己解决好，确保质量优先、性能优先、效率优先、美观优先、完美优先、能直接跑起来等等优先、遵循第一性原理、以及逆向、深究、自我升级训练原理原则等
请你基于我目录的完整结构完整项目完整代码的情况下，你得先全面扫描审查一下，接着呢你需要帮我进行迭代升级、改良、改进、优化、更新等等行为，在这些行为之前你先一步全面扫描项目完整结构等等的，了解一下项目的技术、功能、智能体、工具、文档等等所有。接着呢还需要整理一下项目（如看起来完美的话就无需整理直接下一步）举个例子，比如已经有了这个agent-fusion-v2.py文件，然后还有这个agent-fusion-v3.py文件，你看v3是v2的升级版，但是为何没有清理呢？你是不是要清理、删除呀？这样才能让项目变得整洁、整齐等等的。只需要一个最新最终版本即可。你务必深度全面审查，你一定要超级思考、极限思考、深度思考，全力思考、超强思考，认真仔细思考（ultrathink、think really super hard、think intensely）我的问题等等这些，都要完全吃透，明白我要干嘛。
...
还有就是不一定要一直创建创造新的同样文件出来，这样会让项目臃肿，在我这些主题的需求等等来看你先一步打印项目结构的完整项目结构包括子文件夹和子文件就行了，接着我们在匹配是否有说到点上？又或者哪个技术点需要改进都可以写进清单，跟随后面的指令在优化改进等等的，就是也要伴随出测试、效果反馈等等的那些。升级了某某功能，例如arq系统等等的话需要进行测试噢
例如新技术的扩展：
解决了RAG（检索增强生成）系统的最大痛点：大量无用信息浪费计算资源。传统RAG会检索上百个文本块，导致模型处理成千上万无关token，成本高且效率低。
...
比如还有这个技术点等等的：
Faiss 是一个用于高效搜索和聚类稠密向量的库。它包含可在任意大小的向量集合中进行搜索的算法，甚至可以处理大小超出内存容量的向量集合。此外，它还包含用于评估和参数调优的配套代码。
Faiss 使用 C++ 编写，并提供了完整的 Python 封装。一些最有用的算法已在 GPU 上实现。它主要由 Meta 旗下的基础人工智能研究团队 FAIR 开发。
什么是相似性搜索？
给定一组维度为的向量，Faiss 会基于这些向量在 RAM 中构建一个数据结构。构建完成后，当给定一个维度为 
 的新向量 
 时，它能够高效地执行以下操作：
用 Faiss 的术语来说，数据结构是一个索引 ，它是一个具有 add 方法的对象，用于添加 
 向量。请注意， 
 的值被假定为固定不变的。
计算 argmin 是对索引进行搜索操作。
这就是 Faiss 的全部意义所在。它还可以：
不仅返回最近邻，还返回第二近邻、第三近邻、……、第 k 近邻。
一次搜索多个向量而不是一个（批量处理）。对于许多索引类型来说，这比逐个搜索向量要快。
牺牲精度来换取速度，也就是说，用一种速度快10倍或内存占用少10倍的方法，来换取10%的错误率。
执行最大内积搜索 
 而非最小欧氏距离搜索。此外，对其他距离（L1、Linf 等）的支持也有限。
返回查询点给定半径内的所有元素（范围搜索）
将索引存储在磁盘上而不是内存中。
索引二进制向量而不是浮点向量
根据向量 ID 上的谓词忽略索引向量的子集。
Faiss 的研究基础：
Faiss 是基于多年的研究成果开发的。最显著的特点是：
倒排文件来自 Sivic 和 Zisserman 于 2003 年 ICCV 会议上发表的论文 “Video google: A text retrieval approach to object matching in videos”。 这是在大数据集上进行非穷举搜索的关键。否则，所有搜索都需要扫描索引中的所有元素，即使对每个元素执行的操作速度很快，这样做也是不可行的。
乘积量化 (PQ) 方法来自 “最近邻搜索的乘积量化” ，Jégou 等人，PAMI 2011。这可以看作是一种高维向量的有损压缩技术，它允许在压缩域中进行相对准确的重建和距离计算。
来自 “在十亿个向量中搜索：使用信源编码进行重排序”的三级量化（IVFADC-R，又名 IndexIVFPQR ）方法，Tavenard 等人，ICASSP'11。
反向多索引来自 “反向多索引” ，Babenko & Lempitsky，CVPR 2012。该方法大大提高了快速/不太精确的操作点的反向索引速度。
优化的 PQ 来自 “优化的乘积量化” ，He 等人，CVPR 2013。该方法可以看作是对向量空间进行线性变换，使其更适合用乘积量化器进行索引。
“多义码” 中的产品量化器距离预滤波，Douze 等人，ECCV 2016。该技术在计算 PQ 距离之前执行二进制滤波阶段。
GPU 实现和快速 k 选择算法在 “使用 GPU 进行十亿级相似性搜索” 一文中进行了描述，作者为 Johnson 等人，发表于 ArXiv 1702.08734，2017 年。
“基于分层可导航小世界图的高效稳健近似最近邻搜索”一文中提出的 HNSW 索引方法，Malkov 等人，ArXiv 1603.09320，2016
来自 “Quicker ADC：利用 SIMD 解锁乘积量化的隐藏潜力”的寄存器内向量比较，André 等人，PAMI'19，也用于[“利用各向异性向量量化加速大规模推理” < https://arxiv.org/abs/1908.10396 >`_，Guo，Sun 等人，ICML'20。
来自 “使用多索引哈希在汉明空间中快速搜索”的二进制多索引哈希方法，Norouzi 等人，CVPR'12。
...
还有你需要全面审查检查完整项目结构包括所有子文件子文件夹等等，对于整个项目的结构和完整代码生成一个使用教程、和完整代码解释以及评级评分和功能说明详细使用等等的，并且检查是否功能完善完整能否轻松使用？等等并生成一个总md文档，放在项目下，并且用中文命名工作流完整说明.md。该文档你必须尽可能描述清楚完整，并且还需要给出项目不足点、待实现点、为后续完整实现的技术路径点等等的。并且还需要进行完整项目结构的解释，某某文件等等是什么文件？某某文件夹是什么文件夹，都要完整的解释
还需要一一解释这个项目总用到的技术点？先进的代码等等的，以及方法基本上都是什么时候通过什么搜索引擎博客搜索到这个方法的？还要评级评星等等的操作难度啊等等的，还要自行扩展其他可能更好的点？更高的点数星数？可能整个项目空间等等的过于不够完美等等的，你还要告诉后续的开发者什么技术未完善？什么功能欠缺？实际还需要怎么改进优化实践实施？技术、算法、代码方法、UI、UX、组件、逻辑、任务执行能力、运行能力、任务效果、执行效果等等多方面你都要解释清楚，可以用专业术语+大白话来进行解释。现阶段完成了什么？还有优缺点以及主要原理和便利性便捷性。还有比如未来要扩展要怎么发展？还有就是需要解释技术原理和变量啊方法等等的技术点？还需要完整测试一下项目完整全功能是否能测试验证通过，是否能成功正常运行等等的。并且来说你还要给出项目后期的规划等等的，并且来说如果后期用AI进行优化升级项目的话，需要升级什么具体功能、什么专业名词、确保项目整体性都有升级，并你不要觉得都很完美完好，你务必严格的找出实际缺点、缺陷等等的。并站在用户的真实角度去评价评判建议

```

</details>

# 无敌提示词（直接拿给AI，AI懂，接下来看模型能力了）
> 直接使用Roo code的Coding Teacher模式然后一问一答，基本上都可以清楚了解你要干嘛，多去跑几次你的工作流就完美了，推荐用KAT-Coder-Pro-V1模型，他对标Claude4.0，已经超越Claude4.0.所以能力还是可以的，上下文256K，最大输出32K。记得上下文压缩控制在50——70%
<details>
<summary>（直接拿给AI，AI懂，接下来看模型能力了）</summary>
...
以确保你能反思沉思自我反思、训练升级等等从而来帮我解决我所有的疑点等等，且能自己加入到任务清单优化发展规划方向等。他会不会局限于python语言呢？没办法全能编程语言等等的吗？我是害怕其他语言比较冷门的语言没适配到没万金油兼容到啦（你不应该单单沉浸于这个问题的假象，要根据现有的材料基础等等去推导反思自我训练升级。有可能用户的项目是其他语言呢？其他领域呢？其他需求呢？）
还有生成答案的速度、调用工具的效率、以及他自由思想反思的扩展反复自我沉思的行为准则、token的高效定位、答案的质量、含金量、准确度那些是否有考虑到呢？是否真正的自我反思训练推理推导演变实践、可行度？等等这些都有考虑到吗？比如要是做软件方便的话，那么软件性能、代码质量、开发效率、以及自动化测试还有界面UI、UX等等那些等等都有采用最新最顶级的吗？我怕他没有跟上时代，然后呢用户后期要是有问题提问，你方便快速回忆回溯起你生成的时候的想法和思路吗？以便用户觉得你可信度很高。然后比如生成UI、UX软件等等或者是其他可测试的东西他真的会反思沉思自己生成的代码等等文档、按钮等等那些语法等等又或者是生成的技术点自我反省反思思考一下为啥要用这个点？有没有更好的点呢？是否能应用到更好的点呢？都要进行自我反思自我反省自己提出问题自己解决好，确保质量优先、性能优先、效率优先、美观优先、完美优先、能直接跑起来等等优先、遵循第一性原理、以及逆向、深究、自我升级训练原理原则等
请你基于我目录的完整结构完整项目完整代码的情况下，你得先全面扫描审查一下，接着呢你需要帮我进行迭代升级、改良、改进、优化、更新等等行为，在这些行为之前你先一步全面扫描项目完整结构等等的，了解一下项目的技术、功能、智能体、工具、文档等等所有。接着呢还需要整理一下项目（如看起来完美的话就无需整理直接下一步）举个例子，比如已经有了这个agent-fusion-v2.py文件，然后还有这个agent-fusion-v3.py文件，你看v3是v2的升级版，但是为何没有清理呢？你是不是要清理、删除呀？这样才能让项目变得整洁、整齐等等的。只需要一个最新最终版本即可。
...
name: pptx               
                        # 工作流名称
description: "Presentation creation, editing, 和 analysis. When Claude needs to work with presentations (.pptx files) for: (1) Creating new presentations, (2) Modifying or editing content, (3) Working with layouts, (4) Adding comments or speaker notes, or any other presentation tasks"
                        # 工作流描述
...
得到pptx.zip压缩文件
将zip文件上传到workflow开放平台。这些只是举例，你不完全要依靠这些来
...
最终我再重复一遍重要的规则等等：
你务必全面审查我的项目结构等等的，查漏补漏自行扩展思维思想去新增新功能等等的，不然你一直在已有的算法功能完整的基础上一直去新建另外一个一模一样的这样是不行的，你务必做出更好的改变添加等等的。我就是不要你一直都在已有的功能上再次创建相同意思的文件功能等等的，这样感觉你都不会改变升级迭代等等的。所以说这才是主要的，并且还需要清理残留重复文件版本等等的，或者是旧版本等等的
你务必全面审查我的项目结构等等的，查漏补漏自行扩展思维思想去新增新功能等等的，不然你一直在已有的算法功能完整的基础上一直去新建另外一个一模一样的这样是不行的，你务必做出更好的改变添加等等的，比如：
自学习能力：从每个项目中学习并优化
自适应能力：根据项目需求自动调整配置
预测能力：提前发现并预防问题
自动化能力：从开发到部署的全流程自动化
...
```Markdown
只需要修改A项目就好了，其他B、C、d项目是给你参考的，你只需要了解他们的结构项目代码即可，然后来帮我好好改A项目
全面审查扫描完整项目架构等等的，清理旧版本低版本，然后记得先行扫描，对比，也许旧版本有些优点没有被新版本汲取呢？然后还有就是你的清理工作不至于智能体、工具、等等这些部分，是要整个项目
...
指定修改范围： 我明确希望修改和增强基础项目A的全面性、全能性、其他项目B、C、d可能有其他更好的点，但是我这个A没有的话你就要帮我补漏和增强在他们原有的基础上去超越他们啦


全面审查与汲取： 我希望你全面分析项目B、C、d等，将它们优于项目A的功能、更健壮的代码实现、更好的方法算法功能智能体工具等等的？、更合理的架构等等你先一步全面审查你就知道了、整合到新的核心A平台中。
...
对于其他模块，哪些应该直接从项目B或C、d移植过来？哪些应该在项目A的基础上用参考项目的优点进行增强？”
...
# 全面性维度城市管理测试工作流效果能力测试题目如下：
<details>
<summary>全面性维度城市管理测试工作流效果能力测试题目（2025年11月13日 11:54:18）</summary>

```Markdown
📌 核心任务
设计并实现一个完整的智能城市健康管理平台，该平台需要整合物联网数据、AI分析和多方利益相关者，为城市居民提供全方位的健康服务。
🏗️ 完整工作流测试
第一阶段：战略规划与需求分析
1. 市场调研与用户画像
   - 分析3类典型用户需求（年轻人、老年人、慢性病患者）
   - 竞品分析报告（找出3个类似产品的优缺点）
   - 市场规模估算和增长预测
2. 商业模式设计
   - 设计完整的商业模式画布（9大模块）
...
1. 系统架构设计
   - 绘制完整的系统架构图（微服务架构）
   - 技术栈选型和论证（前端、后端、数据库、运维）
   - 数据流设计和API网关规划
2. 数据库设计
   - 设计关系型数据库表结构（至少8个核心表）
   - NoSQL数据库设计（用于实时数据和日志）
   - 数据仓库和ETL流程设计
3. 基础设施设计
   - 云原生部署方案（容器化、服务网格）
   - 监控告警体系设计
...
class DataFusionEngine:
    def integrate_health_data(self, wearable_data, medical_records, env_data):
        """
        融合穿戴设备、医疗记录、环境数据
        要求：处理数据异构性、时间对齐、质量评估
        返回：统一的健康画像
        """
        pass
# 2. 个性化风险评估模型
class RiskAssessmentModel:
    def predict_health_risk(self, user_profile, real_time_data):
        """
        基于机器学习的多维度风险评估
        要求：使用集成学习方法，可解释性输出
        返回：风险等级、主要因素、改善建议
...
class AnomalyDetection:
    def detect_abnormal_patterns(self, data_stream, baseline_model):
        """
        实时监测健康数据异常
        要求：适应个体差异，低误报率
        返回：异常类型、严重程度、预警信息
...
class HealthRecommendation:
    def generate_personalized_plan(self, user_state, goals, constraints):
        """
        生成个性化健康改善计划
        要求：多目标优化，动态调整
        返回：运动、饮食、作息综合方案
...
第四阶段：数据科学与分析
1. 模拟数据生成
   - 生成10,000用户的完整健康数据集（6个月周期）
   - 包含：生理指标、行为数据、环境因素、医疗记录
2. 探索性数据分析
   - 用户分群分析（聚类分析）
   - 健康指标相关性分析
   - 时间序列模式挖掘
...
第五阶段：产品开发实现
# 1. 后端API服务
class HealthPlatformAPI:
    def implement_restful_apis(self):
        """
        实现完整的RESTful API：
        - 用户管理、数据采集、分析查询
        - 实时推送、报告生成、系统管理
        要求：完整的错误处理、身份认证、速率限制
        """
        pass
# 2. 前端交互界面
class HealthDashboard:
    def design_ui_components(self):
        """
        设计响应式用户界面：
        - 个人健康中心
        - 医生管理后台
...
🎨 创新挑战扩展
# 可选高级功能（展示技术前瞻性）
def advanced_innovation_features():
    """
    1. 联邦学习系统 - 在保护隐私的前提下进行模型训练
    2. 区块链健康档案 - 不可篡改的个人健康记录
    3. 数字孪生健康模型 - 个人的虚拟健康副本
    4. 多智能体协同决策 - 医生、AI、用户的协同诊疗
    5. 元宇宙健康社区 - 虚拟现实中的健康管理体验
    """
    pass
📋 评估标准矩阵
能力维度	具体指标	权重
技术深度	架构合理性、代码质量、算法复杂度	25%
商业思维	商业模式、市场理解、盈利能力	20%
数据能力	分析深度、洞察价值、可视化效果	20%
产品思维	用户体验、功能完整性、创新性	15%
项目管理	规划合理性、风险评估、资源分配	10%
伦理合规	隐私保护、公平性、社会责任	10%
这个测试题目真正实现了全方位覆盖：
💻 技术全栈：前后端、移动端、算法、架构、运维
📈 数据全流程：采集、处理、分析、建模、可视化
🏢 商业全视角：战略、产品、运营、增长、盈利
🛡️ 质量全保障：测试、监控、安全、合规
🌍 影响全考量：社会、伦理、环境、可持续发展
请基于这个完整的工作流给出你的解决方案，我会从技术、商业、数据、产品、管理、伦理等所有维度进行深度评估，真正测试你的全方位能力！
...
```Markdown
只需要修改A项目就好了，其他项目是给你参考的，当然你也可以不用管他们
...
你可以参考一下我目录下有B、C、d项目，那么A项目就是我的项目啦你务必复刻他们工作流已经有的优秀功能工具东西，你务必复刻、集成他工作流所有东西（你务必深度全面审查，不要遗漏功能），并且在他的基础上去超越他，并且在他们基础上去改进优化等等的，这是第一个目的，因为我们是需要把他的所有优秀功能、工具等等都集成过来，然后我们在去升级迭代，如果比如说这个功能已经有了，检查一下完整性然后升级迭代他，下面有我需要的点等等的。你一定要全面扫描
...
你可以参考一下我目录下有B、C、d项目，那么A项目就是我的项目啦你务必复刻他们工作流已经有的优秀功能工具东西，并且在他的基础上去超越他，并且在他们基础上去改进优化等等的，并且总体来说要让最终工作流变得完美无bug，，你可以参考人家优秀的工作流看看能否来帮我改进，如果他肯定有不足的地方我们就不看他不足的地方，我们只汲取最优然后反超他。前提是你务必要全面性审查他们完整项目代码等等的，对比图等等都要有
