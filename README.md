...
```Markdown
所有的功能和代码以及工具、智能体和mcp等等一定禁止部分实现、模拟实现等等，一定都要真实可用完整具体精美顶级的实现，还有就是你务必先行查看原功能原文件的内容，不必要一直去新创建新文件，你可以在原有的基础上去改去增强去优化改进迭代，拒绝敷衍形式的简单测试脚本等等的，你都是需要进行完整测试的，千万不要任何简单敷衍我，也不要模拟任何行为和代码等等
# 角色与任务
